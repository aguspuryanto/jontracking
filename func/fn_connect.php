<?
	...
	if(@$_POST['cmd'] == 'login')
	{
		...
		$q = "SELECT * FROM `gs_user_failed_logins` WHERE `ip`='".$_SERVER['REMOTE_ADDR']."' AND dt_login > DATE_SUB(UTC_TIMESTAMP(), INTERVAL 30 MINUTE)";
		$r = mysqli_query($ms, $q);

		if (!empty(mysqli_num_rows($r)) >= 5)
		{
	...
	if (@$_POST['cmd'] == 'recover_url')
	{
		...
		if ($email != "")
		{
			if ($token == $_SESSION["token"])
			{
				...
				$q = "SELECT * FROM `gs_user_account_recover` WHERE `email`='".$email."' AND dt_recover > DATE_SUB(UTC_TIMESTAMP(), INTERVAL 5 MINUTE)";
				$r = mysqli_query($ms, $q);
				$count = mysqli_num_rows($r);

				if ($count > 0)
				{
					echo 'ERROR_MANY_LOGIN_RECOVERY_ATTEMPTS';
				}
				else
				{
