<?
	...
	function getRoute($user_id, $imei, $dtf, $dtt, $min_stop_duration, $filter)
	{
		...
		if (count($route) > 0)
		{
            ...
            if(is_array($odo_sensor)) {
                $odo_sensor = $odo_sensor[0];
            }
			...
			// count avg speed
			$avg_speed_drives_duration_time = 0;
			for ($i=0; $i<count($result['drives']); ++$i)
			{
				$diff = strtotime($result['drives'][$i][5])-strtotime($result['drives'][$i][4]);
				$avg_speed_drives_duration_time += $diff;
			}
			$result['avg_speed'] = getRouteAvgSpeed($result['route_length'], $avg_speed_drives_duration_time);

			// count fuel consumption
			$result['fuel_consumption'] = 0;
			if ($accuracy['ign_fuel_cons_stops'] == true)
			{
