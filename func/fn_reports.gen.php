<?
	...
	if (version_compare(PHP_VERSION, '5.5.0', '>=')) { include ('../tools/html2pdf.php'); } else { include ('../tools/html2pdf080.php'); }
	...
	function reportsGenerate()
	{
		...
		$markers_addresses = $_POST['markers_addresses'];
		...
		{
			$default = array('dn_starts_hour' => '22',
							 'dn_starts_minute' => '00',
							 'dn_ends_hour' => '06',
							 'dn_ends_minute' => '00');
			...
			else
			{
				...
				if (!isset($other["dn_starts_hour"])) { $other["dn_starts_hour"] = $default["dn_starts_hour"]; }
				if (!isset($other["dn_starts_minute"])) { $other["dn_starts_minute"] = $default["dn_starts_minute"]; }
				if (!isset($other["dn_ends_hour"])) { $other["dn_ends_hour"] = $default["dn_ends_hour"]; }
				if (!isset($other["dn_ends_minute"])) { $other["dn_ends_minute"] = $default["dn_ends_minute"]; }
			}
		}
		...
		else if ($format == 'pdf')
		{
			...
			$report_html .= '<img class="logo" src="data:image/jpg;base64,'.$img_base64.'" /></br><hr/>';
		}
		
		$report_html .= reportsGenerateLoop($type, $imeis, $dtf, $dtt, $ignore_empty_reports, $speed_limit, $stop_duration, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $marker_ids, $zone_ids, $sensor_names, $data_items, $other);
		...
		if ($marker_ids != '')
		{
			$markers = count(explode(",", $marker_ids));
		}
		else
		{
			$markers = 0;
		...
		if(is_file($file_path))
		{
			...
			// in case MySQL connection is closed due to long report generation - open it again
			if (!$r)
			{
	...
	function reportsGenerateLoop($type, $imeis, $dtf, $dtt, $ignore_empty_reports, $speed_limit, $stop_duration, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $marker_ids, $zone_ids, $sensor_names, $data_items, $other)
	{
		global $la;
		...
		for ($i=0; $i<count($imeis); ++$i)
		{
			...
			else if ($type == "route_data_sensors") //ROUTE DATA WITH SENSORS
			{
				...
				$report = reportsGenerateRouteDataSensors($imei, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $sensors_, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items);
				
				if ($report == false)
				{
					if ($ignore_empty_reports == 'false')
					{
						$result .= '<h3>'.$la['ROUTE_DATA_WITH_SENSORS'].'</h3>';
						$result .= reportsAddReportHeader($imei, $dtf, $dtt);
						$result .= '<table><tr><td>'.$la['NOTHING_HAS_BEEN_FOUND_ON_YOUR_REQUEST'].'</td></tr></table>';
						$result .= '<br/><hr/>';
					}
			...
			{
				$report = reportsGenerateDrivesAndStops($imei, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $stop_duration, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items);
			...
			else if ($type == "drives_stops_sensors") //DRIVES_AND_STOPS_WITH_SENSORS
			{
				$sensors = getSensors($imei);
				$sensors_ = array();
				...
				$report = reportsGenerateDrivesAndStopsSensors($imei, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $sensors_, $stop_duration, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items);
			...
			else if ($type == "drives_stops_logic") //DRIVES_AND_STOPS_WITH_LOGIC_SENSORS
			{
				...
				$report = reportsGenerateDrivesAndStopsLogicSensors($imei, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $sensors_, $stop_duration, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items);
			...
			{
				$report = reportsGenerateTravelSheet($imei, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $stop_duration, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items);
			...
			{
				$report = reportsGenerateTravelSheetDayNight($imei, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $stop_duration, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items, $other);
			...
			{
				$report = reportsGenerateOverspeed($imei, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $speed_limit, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items);
			...
			{
				$report = reportsGenerateUnderspeed($imei, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $speed_limit, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items);
			...
			else if ($type == "marker_in_out") //MARKER_IN_OUT
			{
				$report = reportsGenerateMarkerInOut($imei, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $marker_ids, $data_items);
				
				if ($report == false)
				{
					if ($ignore_empty_reports == 'false')
					{
						$result .= '<h3>'.$la['MARKER_IN_OUT'].'</h3>';
						$result .= reportsAddReportHeader($imei, $dtf, $dtt);
						$result .= '<table><tr><td>'.$la['NOTHING_HAS_BEEN_FOUND_ON_YOUR_REQUEST'].'</td></tr></table>';
						$result .= '<br/><hr/>';
					}
			...
			else if ($type == "marker_in_out_gen") //MARKER_IN_OUT_WITH_GEN_INFO
			{
				$report = reportsGenerateMarkerInOutGenInfo($imei, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $speed_limit, $stop_duration, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $marker_ids, $data_items);
				
				if ($report == false)
				{
					if ($ignore_empty_reports == 'false')
					{
						$result .= '<h3>'.$la['MARKER_IN_OUT_WITH_GEN_INFORMATION'].'</h3>';
						$result .= reportsAddReportHeader($imei, $dtf, $dtt);
						$result .= '<table><tr><td>'.$la['NOTHING_HAS_BEEN_FOUND_ON_YOUR_REQUEST'].'</td></tr></table>';
						$result .= '<br/><hr/>';
					}
			...
			{
				$report = reportsGenerateZoneInOut($imei, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $zone_ids, $data_items);
			...
			{
				$report = reportsGenerateZoneInOutGenInfo($imei, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $speed_limit, $stop_duration, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $zone_ids, $data_items);
			...
			{
				$report = reportsGenerateEvents($imei, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items);
			...
			{
				$report = reportsGenerateFuelFillings($imei, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items);
			...
			{
				$report = reportsGenerateFuelThefts($imei, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items);
			...
			else if ($type == "logic_sensors") //LOGIC_SENSORS
			{
				...
				$report = reportsGenerateLogicSensorInfo($imei, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $sensors_, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items);
			...
			{
				$report = reportsGenerateImageGallery($imei, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items);
		...
		else if ($type == "current_position") //CURRENT POSITION
		{
			...
			$result .= reportsAddReportHeader('', $dtf, $dtt);
			$result .= reportsGenerateCurrentPosition($imeis, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items, false);
			$result .= '<br/><hr/>';
		}
		else if ($type == "current_position_off") //CURRENT POSITION OFFLINE
		{
			...
			$result .= reportsAddReportHeader('', $dtf, $dtt);
			$result .= reportsGenerateCurrentPosition($imeis, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items, 'offline');
			$result .= '<br/><hr/>';
		}
		else if ($type == "overspeed_count") //OVERSPEED COUNT
		{
			$result .= '<h3>'.$la['OVERSPEED_COUNT_MERGED'].'</h3>';
			$result .= reportsAddReportHeader('', $dtf, $dtt);
			$result .= reportsGenerateOverspeedCountMerged($imeis, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $speed_limit, $data_items);
			$result .= '<br/><hr/>';
		}
		else if ($type == "underspeed_count") //UNDERSPEED COUNT
		{
			$result .= '<h3>'.$la['UNDERSPEED_COUNT_MERGED'].'</h3>';
			$result .= reportsAddReportHeader('', $dtf, $dtt);
			$result .= reportsGenerateUnderspeedCountMerged($imeis, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $speed_limit, $data_items);
			$result .= '<br/><hr/>';
		}		
		...
		elseif ($type == "tasks") //TASKS
		{
			...
			$result .= reportsAddReportHeader('', $dtf, $dtt);
			$result .= reportsGenerateTasks($imeis, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items);
			$result .= '<br/><hr/>';
		}
		elseif ($type == "rilogbook") //RFID_AND_IBUTTON_LOGBOOK
		{
			...
			$result .= reportsAddReportHeader('', $dtf, $dtt);
			$result .= reportsGenerateRiLogbook($imeis, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items);
			$result .= '<br/><hr/>';
		}
		elseif ($type == "dtc") //DIAGNOSTIC_TROUBLE_CODES
		{
			...
			$result .= reportsAddReportHeader('', $dtf, $dtt);
			$result .= reportsGenerateDTC($imeis, convUserUTCTimezone($dtf), convUserUTCTimezone($dtt), $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items);
			$result .= '<br/><hr/>';
		}
	...
	function reportsGenerateGenInfo($imei, $dtf, $dtt, $speed_limit, $stop_duration, $data_items) //GENERAL_INFO
	{
		...
		if (in_array("custom_fields", $data_items))
		{
	...
	function reportsGenerateObjectInfo($imeis, $data_items)
	{
		...
		$q = "SELECT * FROM `gs_user_object_groups` WHERE `user_id`='".$user_id."' ORDER BY `group_id` ASC";
		$r = mysqli_query($ms, $q);
		...
		for ($i=0; $i<count($imeis); ++$i)
		{
			...
			$q = "SELECT gs_objects.*, gs_user_objects.*
				...
				WHERE gs_user_objects.user_id='".$user_id."' AND gs_user_objects.imei='".$imei."'";
			$r = mysqli_query($ms, $q);
			...
			if (in_array("group", $data_items))
			{
				...
				{
					$group_name = $groups[$row['group_id']];
				}
	...
	function reportsGenerateCurrentPosition($imeis, $dtf, $dtt, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items, $status)
	{
		global $ms, $_SESSION, $gsValues, $la, $user_id;
		...
		for ($i=0; $i<count($imeis); ++$i)
		{
			...
			while($row = mysqli_fetch_array($r))
			{
				...
				{					
					...
					if (in_array("position", $data_items))
					{
						$result .= '<td>'.reportsGetPossition($lat, $lng, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses).'</td>';
					}
	...
	function reportsGenerateRouteDataSensors($imei, $dtf, $dtt, $sensors, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items)	
	{
		...
		{
			$sensor = $sensors[$k];
			if (($sensors[$k]['result_type'] == 'logic') || ($sensors[$k]['result_type'] == 'value') || ($sensors[$k]['result_type'] == 'percentage'))
			{
				$result .= '<th>'.$sensor['name'].'</th>';	
			}			
		}
		...
		for ($i=0; $i<count($route); ++$i)
		{
			...
			if (isset($prev_dt_tracker))
			{
				$dt_difference = strtotime($dt_tracker) - strtotime($prev_dt_tracker);
			...
			if (($i == 0) || ($i == count($route)-1) || ($is_interval == true))
			{
				$prev_dt_tracker = $dt_tracker;
				$is_interval = false;
	...
	function reportsGenerateDrivesAndStops($imei, $dtf, $dtt, $stop_duration, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items) //DRIVES_AND_STOPS
	{
		global $la, $user_id;
		...
		{			
			for ($j=0; $j<count($data['stops']); ++$j)
			{
				if ($data['stops'][$j][6] == $dt_sort[$i])
				{
					...
					$result .= '<td colspan="3">'.reportsGetPossition($lat, $lng, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses).'</td>';
					...
					if (in_array("driver", $data_items))
					{
						$params = $data['route'][$data['stops'][$j][0]][6];
						$driver = getObjectDriver($user_id, $imei, $params);
						if ($driver['driver_name'] == ''){$driver['driver_name'] = $la['NA'];}
	...
	function reportsGenerateDrivesAndStopsSensors($imei, $dtf, $dtt, $sensors, $stop_duration, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items) //DRIVES_AND_STOPS_WITH_SENSORS
	{
		global $la, $user_id;
	...
	function reportsGenerateDrivesAndStopsLogicSensors($imei, $dtf, $dtt, $sensors, $stop_duration, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items) //DRIVES_AND_STOPS_WITH_LOGIC_SENSORS
	{
		global $la, $user_id;
	...
	function reportsGenerateTravelSheet($imei, $dtf, $dtt, $stop_duration, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items) //TRAVEL_SHEET
	{
		global $la, $user_id;
		...
		// get odo sensor
		$odo_sensor = getSensorFromType($imei, 'odo');
		...
		if (in_array("odometer_a", $data_items))
		{
			$result .= '<th>'.$la['ODOMETER_A'].'</th>';
		}
		...
		if (in_array("odometer_b", $data_items))
		{
			$result .= '<th>'.$la['ODOMETER_B'].'</th>';
		}
		...
		{			
			...
			$params_a = $data['route'][$route_id_a][6];
			$params_b = $data['route'][$route_id_b][6];
			
			// this prevents double geocoder calling
			if(!isset($position_a))
			{
				$position_a = reportsGetPossition($lat1, $lng1, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses);
			}
			else
			{
			...
			$position_b = reportsGetPossition($lat2, $lng2, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses);			
			
			if ($odo_sensor)
			{
				if ($odo_sensor[0]['result_type'] == 'abs')
				{
					$odometer_a = getSensorValue($params_a , $odo_sensor[0]);
					$odometer_a = convDistanceUnits($odometer_a['value'], 'km', $_SESSION["unit_distance"]);
					$odometer_a = round($odometer_a * 100) / 100;
					$odometer_a = $odometer_a.' '.$la["UNIT_DISTANCE"];	
				}
				else
				{
					$odometer_a = $la['NA'];
			...
			if ($odo_sensor)
			{
				if ($odo_sensor[0]['result_type'] == 'abs')
				{
					$odometer_b = getSensorValue($params_b , $odo_sensor[0]);
					$odometer_b = convDistanceUnits($odometer_b['value'], 'km', $_SESSION["unit_distance"]);
					$odometer_b = round($odometer_b * 100) / 100;
					$odometer_b = $odometer_b.' '.$la["UNIT_DISTANCE"];
				}
				else
				{
					$odometer_b = $la['NA'];
	...
	function reportsGenerateTravelSheetDayNight($imei, $dtf, $dtt, $stop_duration, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items, $other) //TRAVEL_SHEET_DAY_NIGHT
	{
		global $la, $user_id;
		...
		if (($total_night_fuel_consumption > 0) && ($total_night_route_length > 0))
		{
			$total_night_avg_fuel_consumption = ($total_night_fuel_consumption / $total_night_route_length) * 100;				
			$total_night_avg_fuel_consumption = round($total_night_avg_fuel_consumption * 100) / 100;
		}
		
		if (($total_day_fuel_consumption > 0) && ($total_day_route_length > 0))
		{
			$total_day_avg_fuel_consumption = ($total_day_fuel_consumption / $total_day_route_length) * 100;				
			$total_day_avg_fuel_consumption = round($total_day_avg_fuel_consumption * 100) / 100;
		}
	...
	function reportsGenerateMileageDaily($imei, $dtf, $dtt, $data_items) //MILEAGE_DAILY
	{
		...
		for ($i=0; $i<count($dates)-1; ++$i)
		{
			...
			{				
				...
				if (in_array("move_duration", $data_items))
				{
					$rows .= '<td>'.$data['drives_duration'].'</td>';
					$total_drives_duration += $data['drives_duration_time'];
				}				
		...
		else
		{
			...
			if (in_array("total", $data_items))
			{
				...
				if (in_array("move_duration", $data_items))
				{
					$result .= '<td>'.getTimeDetails($total_drives_duration, false).'</td>';
				}
	...
	function reportsGenerateOverspeed($imei, $dtf, $dtt, $speed_limit, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items) //OVERSPEED
	{
		global $la, $user_id;
		...
		for ($i=0; $i<count($overspeeds); ++$i)
		{
			...
			if (in_array("overspeed_position", $data_items))
			{
				$result .= '<td>'.reportsGetPossition($overspeeds[$i][5], $overspeeds[$i][6], $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses).'</td>';
			}
			
			if (in_array("driver", $data_items))
			{
				$params = $overspeeds[$i][7];
				$driver = getObjectDriver($user_id, $imei, $params);
				if ($driver['driver_name'] == ''){$driver['driver_name'] = $la['NA'];}
	...
	function reportsGenerateOverspeedCountMerged($imeis, $dtf, $dtt, $speed_limit, $data_items) //OVERSPEED_COUNT
	{
		...
		for ($i=0; $i<count($imeis); ++$i)
		{
			...
			$overspeeds = getRouteOverspeeds($route['route'], $speed_limit);
											
			if (count($route['route']) == 0)
			{
				$result .= '<tr align="center">';
				$result .= '<td>'.getObjectName($imei).'</td>';
				$result .= '<td colspan="7">'.$la['NOTHING_HAS_BEEN_FOUND_ON_YOUR_REQUEST'].'</td>';
				$result .= '</tr>';
			}
			else
			{
				...
				if (in_array("route_start", $data_items))
				{
					$result .= '<td>'.$route['route'][0][0].'</td>';
				}
				
				if (in_array("route_end", $data_items))
				{
					$result .= '<td>'.$route['route'][count($route['route'])-1][0].'</td>';
				}
				
				if (in_array("route_length", $data_items))
				{
					$result .= '<td>'.$route['route_length'].' '.$la["UNIT_DISTANCE"].'</td>';					
				}
				
				if (in_array("move_duration", $data_items))
				{
					$result .= '<td>'.$route['drives_duration'].'</td>';					
				}
				
				if (in_array("top_speed", $data_items))
				{
					$result .= '<td>'.$route['top_speed'].' '.$la["UNIT_SPEED"].'</td>';					
				}
				
				if (in_array("avg_speed", $data_items))
				{
					$result .= '<td>'.$route['avg_speed'].' '.$la["UNIT_SPEED"].'</td>';					
				}
				
				if (in_array("overspeed_count", $data_items))
				{
					$result .= '<td>'.count($overspeeds).'</td>';					
				}
	...
	function reportsGenerateUnderspeed($imei, $dtf, $dtt, $speed_limit, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items) //UNDERSPEED
	{
		global $la, $user_id;
		...
		for ($i=0; $i<count($underpeeds); ++$i)
		{
			...
			if (in_array("underspeed_position", $data_items))
			{
				$result .= '<td>'.reportsGetPossition($underpeeds[$i][5], $underpeeds[$i][6], $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses).'</td>';
			}
			
			if (in_array("driver", $data_items))
			{
				$params = $underpeeds[$i][7];
				$driver = getObjectDriver($user_id, $imei, $params);
				if ($driver['driver_name'] == ''){$driver['driver_name'] = $la['NA'];}
	...
	function reportsGenerateUnderspeedCountMerged($imeis, $dtf, $dtt, $speed_limit, $data_items) //UNDERSPEED_COUNT
	{
		global $ms, $_SESSION, $la, $user_id;
		...
		if (in_array("underspeed_count", $data_items))
		{
			$result .= '<th>'.$la['UNDERSPEED_COUNT'].'</th>';
		}
		...
		for ($i=0; $i<count($imeis); ++$i)
		{
			...
			$accuracy = getObjectAccuracy($imei);		
			$route = getRoute($user_id, $imei, $dtf, $dtt, 1, true);
			$underspeeds = getRouteUnderspeeds($route['route'], $speed_limit);
			...
			else
			{
				...
				if (in_array("underspeed_count", $data_items))
				{
					$result .= '<td>'.count($underspeeds).'</td>';					
				}
	...
	function reportsGenerateMarkerInOut($imei, $dtf, $dtt, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $marker_ids, $data_items) //MARKER_IN_OUT
	{
		global $ms, $_SESSION, $la, $user_id;
		
		$marker_ids = explode(",", $marker_ids);
		...
		$q = "SELECT * FROM `gs_user_markers` WHERE `user_id`='".$user_id."'";
		$r = mysqli_query($ms, $q);
		$markers = array();
		
		while($row=mysqli_fetch_array($r))
		{
			if(in_array($row['marker_id'], $marker_ids))
			{
				$markers[] = array($row['marker_id'],$row['marker_name'], $row['marker_lat'], $row['marker_lng'], $row['marker_radius']);	
			}
		}
		
		if ((count($route) == 0) || (count($markers) == 0))
		{
			return false;
		}
		...
		$in_markers = array();
		$in_marker = 0;
		$in_marker_route_length = 0;
		$in_marker_engine_hours = 0;
		$in_markers_total = array();
		...
		for ($i=0; $i<count($route); ++$i)
		{
			$point_dt_tracker = $route[$i][0];
			$point_lat = $route[$i][1];
			$point_lng = $route[$i][2];
			$point_params = $route[$i][6];
			
			for ($j=0; $j<count($markers); ++$j)
			{
				$marker_id = $markers[$j][0];
				$marker_name = $markers[$j][1];
				$marker_lat = $markers[$j][2];
				$marker_lng = $markers[$j][3];
				$marker_radius = $markers[$j][4];
				
				$dist = getLengthBetweenCoordinates($point_lat, $point_lng, $marker_lat, $marker_lng);
				$dist = convDistanceUnits($dist, 'km', $_SESSION["unit_distance"]);
				...
				if ($dist <= $marker_radius)
				{
					...
					if ($in_marker == 0)
					{
						$in_marker_start = $route[$i][0];
						$in_marker_name = $marker_name;
						$in_marker_lat = $point_lat;
						$in_marker_lng = $point_lng;
						$in_marker = $marker_id;
					}
					
					if (isset($prev_point_lat) && isset($prev_point_lng))
					{
						$in_marker_route_length += getLengthBetweenCoordinates($prev_point_lat, $prev_point_lng, $point_lat, $point_lng);
						
						if ($acc !== false)
						{				
							if (isset($prev_point_params[$acc]) && isset($point_params[$acc]))
							{
								if (($prev_point_params[$acc] == '1') && ($point_params[$acc] == '1'))
								{	
									$in_marker_engine_hours += strtotime($point_dt_tracker)-strtotime($prev_point_dt_tracker);									
								}
							}	
						}
					}
				...
				{
					if ($in_marker == $marker_id)
					{
						$in_marker_end = $route[$i][0];
						$in_marker_duration = strtotime($in_marker_end) - strtotime($in_marker_start);
						
						$in_marker_route_length = convDistanceUnits($in_marker_route_length, 'km', $_SESSION["unit_distance"]);						
						$in_marker_route_length = (round($in_marker_route_length * 100) / 100);
						
						$in_markers[] = array(	$in_marker_start,
												$in_marker_end,
												$in_marker_duration,
												$in_marker_route_length,
												$in_marker_engine_hours,
												$in_marker_name,
												$in_marker_lat,
												$in_marker_lng
												);
						
						if (isset($in_marker_total[$marker_id]))
						{
							$in_markers_total[$marker_id]['duration'] += $in_marker_duration;
							$in_markers_total[$marker_id]['route_length'] += $in_marker_route_length;
							$in_markers_total[$marker_id]['engine_hours'] += $in_marker_engine_hours;
						}
						else
						{
							$in_markers_total[$marker_id] = array('name' => $in_marker_name, 'duration' => $in_marker_duration, 'route_length' => $in_marker_route_length, 'engine_hours' => $in_marker_engine_hours);	
						}						
			...
			$prev_point_dt_tracker = $point_dt_tracker;
			$prev_point_lat = $point_lat;
			$prev_point_lng = $point_lng;
			$prev_point_params = $point_params;
		}
		
		// add last marker record if it did not leave
		if ($in_marker != 0)
		{			
			$in_marker_end = $route[$i-1][0];
			$in_marker_duration = strtotime($in_marker_end) - strtotime($in_marker_start);
			...
			if (isset($in_markers_total[$marker_id]))
			{
				$in_markers_total[$marker_id]['duration'] += $in_marker_duration;
				$in_markers_total[$marker_id]['route_length'] += $in_marker_route_length;
				$in_markers_total[$marker_id]['engine_hours'] += $in_marker_engine_hours;
		...
		if (in_array("marker_in", $data_items))
		{
			$result .= '<th>'.$la['MARKER_IN'].'</th>';
		}
		
		if (in_array("marker_out", $data_items))
		{
			$result .= '<th>'.$la['MARKER_OUT'].'</th>';
		}
		...
		if (in_array("marker_name", $data_items))
		{
			$result .= '<th>'.$la['MARKER_NAME'].'</th>';
		}
		
		if (in_array("marker_position", $data_items))
		{
			$result .= '<th>'.$la['MARKER_POSITION'].'</th>';
		}
		
		$result .= '</tr>';
		
		for ($i=0; $i<count($in_markers); ++$i)
		{
			$result .= '<tr align="center">';
			
			if (in_array("marker_in", $data_items))
			{
				$result .= '<td>'.$in_markers[$i][0].'</td>';
			}
			
			if (in_array("marker_out", $data_items))
			{
				$result .= '<td>'.$in_markers[$i][1].'</td>';
			}
			
			if (in_array("duration", $data_items))
			{
				$result .= '<td>'.getTimeDetails($in_markers[$i][2], true).'</td>';
			}
			
			if (in_array("route_length", $data_items))
			{
				$result .= '<td>'.$in_markers[$i][3].' '.$la['UNIT_DISTANCE'].'</td>';
			}
			
			if (in_array("engine_hours", $data_items))
			{
				$result .= '<td>'.getTimeDetails($in_markers[$i][4], true).'</td>';
			}
			
			if (in_array("marker_name", $data_items))
			{
				$result .= '<td>'.$in_markers[$i][5].'</td>';
			}
			
			if (in_array("marker_position", $data_items))
			{
				$result .= '<td>'.reportsGetPossition($in_markers[$i][6], $in_markers[$i][7], $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses).'</td>';
			}
		...
		if (in_array("total", $data_items))
		{
			...
			usort($in_markers_total, function($a, $b) {
				return strcmp($a["name"], $b["name"]);
			});
			
			$result .= '<table>';
			foreach ($in_markers_total as $key=>$value)
			{
				$result .= '<tr>
					<td><strong>'.$value['name'].':</strong></td>
					<td>'.getTimeDetails($value['duration'], true).';</td>
					<td>'.$value['route_length'].' '.$la['UNIT_DISTANCE'].';</td>
					<td>'.getTimeDetails($value['engine_hours'], true).'.</td>
				</tr>';
			}
			$result .= '</table>';
		}
	...
	function reportsGenerateMarkerInOutGenInfo($imei, $dtf, $dtt, $speed_limit, $stop_duration, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $marker_ids, $data_items) //MARKER_IN_OUT_WITH_GEN_INFO
	{
		global $ms, $_SESSION, $la, $user_id;
	...
	function reportsGenerateZoneInOut($imei, $dtf, $dtt, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $zone_ids, $data_items) //ZONE_IN_OUT
	{
		global $ms, $_SESSION, $la, $user_id;
					...
					if (isset($prev_point_lat) && isset($prev_point_lng))
					{
						...
						{				
							if (isset($prev_point_params[$acc]) && isset($point_params[$acc]))
							{
								if (($prev_point_params[$acc] == '1') && ($point_params[$acc] == '1'))
								{	
									$in_zone_engine_hours += strtotime($point_dt_tracker)-strtotime($prev_point_dt_tracker);									
								}
							}	
						}
					}
				...
				else
				{
					if ($in_zone == $zone_id)
					{
						...
						if (isset($in_zones_total[$zone_id]))
						{
							...
							$in_zones_total[$zone_id]['engine_hours'] += $in_zone_engine_hours;
						}
						else
						{
							$in_zones_total[$zone_id] = array('name' => $in_zone_name, 'duration' => $in_zone_duration, 'route_length' => $in_zone_route_length, 'engine_hours' => $in_zone_engine_hours);	
						}						
		...
		for ($i=0; $i<count($in_zones); ++$i)
		{
			...
			if (in_array("engine_hours", $data_items))
			{
				$result .= '<td>'.getTimeDetails($in_zones[$i][4], true).'</td>';
			}
			
			if (in_array("zone_name", $data_items))
			{
				$result .= '<td>'.$in_zones[$i][5].'</td>';
			}
			
			if (in_array("zone_position", $data_items))
			{
				$result .= '<td>'.reportsGetPossition($in_zones[$i][6], $in_zones[$i][7], $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses).'</td>';
			}
	...
	function reportsGenerateZoneInOutGenInfo($imei, $dtf, $dtt, $speed_limit, $stop_duration, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $zone_ids, $data_items) //ZONE_IN_OUT_WITH_GEN_INFO
	{
		global $ms, $_SESSION, $la, $user_id;
	...
	function reportsGenerateEvents($imei, $dtf, $dtt, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items) //EVENTS
	{
		global $ms, $_SESSION, $la, $user_id;
		...
		while($event_data=mysqli_fetch_array($r))
		{
			...
			if (in_array("event_position", $data_items))
			{
				$result .= '<td>'.reportsGetPossition($event_data['lat'], $event_data['lng'], $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses).'</td>';
			}
	...
	function reportsGenerateFuelFillings($imei, $dtf, $dtt, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items) //FUEL_FILLINGS
	{
		global $la, $user_id;
	...
	function reportsGenerateFuelThefts($imei, $dtf, $dtt, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items) //FUEL_THEFTS
	{
		global $la, $user_id;
	...
	function reportsGenerateLogicSensorInfo($imei, $dtf, $dtt, $sensors, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items) //LOGIC_SENSORS
	{
		...
		for ($i=0; $i<count($lsi); ++$i)
		{
			...
			if (in_array("activation_position", $data_items))
			{
				$result .= '<td>'.reportsGetPossition($lsi_activation_lat, $lsi_activation_lng, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses).'</td>';
			}
			
			if (in_array("deactivation_position", $data_items))
			{
				$result .= '<td>'.reportsGetPossition($lsi_deactivation_lat, $lsi_deactivation_lng, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses).'</td>';
			}
	...
	function reportsGenerateTasks($imeis, $dtf, $dtt, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items)
	{
		global $ms, $_SESSION, $la, $user_id;
	...
	function reportsGenerateRiLogbook($imeis, $dtf, $dtt, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items)
	{
		global $ms, $_SESSION, $la, $user_id;
		...
		while($row = mysqli_fetch_array($r))
		{
			...
			if (in_array("position", $data_items))
			{
				$position = reportsGetPossition($lat, $lng, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses);
	...
	function reportsGenerateDTC($imeis, $dtf, $dtt, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items)
	{
		global $ms, $_SESSION, $la, $user_id;
	...
	function reportsGenerateImageGallery($imei, $dtf, $dtt, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses, $data_items) //IMAGE_GALLERY
	{
		...
		while($row=mysqli_fetch_array($r))
		{
			...
			if (in_array("position", $data_items))
			{
				$result .= '<tr>
						...
						<td>'.reportsGetPossition($row['lat'], $row['lng'], $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses).'</td>
					</tr>';
			}
	...
	$markers_addr = array();
	$markers_addr_loaded = false;
	$zones_addr = array();
	$zones_addr_loaded = false;
	
	function reportsGetPossition($lat, $lng, $show_coordinates, $show_addresses, $markers_addresses, $zones_addresses)
	{
		global $ms, $_SESSION, $user_id, $markers_addr, $markers_addr_loaded, $zones_addr, $zones_addr_loaded;
		...
		if ($markers_addresses == 'true')
		{
			if ($markers_addr_loaded == false)
			{
				$q = "SELECT * FROM `gs_user_markers` WHERE `user_id`='".$user_id."'";
				$r = mysqli_query($ms, $q);
				...
				while($row=mysqli_fetch_array($r))
				{
					$markers_addr[] = array($row['marker_id'],$row['marker_name'], $row['marker_lat'], $row['marker_lng'], $row['marker_radius']);	
				}
				
				$markers_addr_loaded = true;
			}
			
			for ($j=0; $j<count($markers_addr); ++$j)
			{
				$marker_name = $markers_addr[$j][1];
				$marker_lat = $markers_addr[$j][2];
				$marker_lng = $markers_addr[$j][3];
				$marker_radius = $markers_addr[$j][4];
				
				$dist = getLengthBetweenCoordinates($lat, $lng, $marker_lat, $marker_lng);
				$dist = convDistanceUnits($dist, 'km', $_SESSION["unit_distance"]);
