    ...
    if(@$_GET['cmd'] == 'load_object_list')
	{
		...
		if ($r)
		{
			...
			while($row = mysqli_fetch_array($r))
			{
				...
				$response->rows[$i]['cell']=array($row['name'],$row['imei'],$active,$expires_on,$row['device'],$row['sim_number'],$last_connection,$protocol,$net_protocol,$port,$status,$used_in,$modify);
				$i++;
			}
		}
