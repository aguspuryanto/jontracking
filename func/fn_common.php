<?
	...
	function getUserData($id)
	{
		...
		if ($privileges["type"] == 'subuser')
		{
			...
			if ($manager_privileges["kml"] == false) { $privileges["kml"] = false; }
			if ($manager_privileges["dashboard"] == false) { $privileges["dashboard"] = false; }
			if ($manager_privileges["history"] == false) { $privileges["history"] = false; }
			if ($manager_privileges["reports"] == false) { $privileges["reports"] = false; }
			if ($manager_privileges["tachograph"] == false) { $privileges["tachograph"] = false; }
			...
			if ($manager_privileges["expenses"] == false) { $privileges["expenses"] = false; }
			...
			$result['privileges_map_arcgis'] = $manager_privileges["map_arcgis"];
			$result["privileges_map_yandex"] = $manager_privileges["map_yandex"];

			$result["privileges_kml"] = $privileges["kml"];
		...
		else
		{
			...
			$result["privileges_map_arcgis"] = $privileges["map_arcgis"];
	...
	function delUser($id)
	{
		...
		$q = "SELECT * FROM `gs_users` WHERE `id`='".$id."' LIMIT 1";
		$r = mysqli_query($ms, $q);
		$row = mysqli_fetch_array($r);
		...
		{
			//write log
			writeLog('user_access', 'Delete user: successful. E-mail: '.$row['email']);
	...
	function checkUserPrivilegesArray($privileges)
	{
		...
		if (!isset($privileges["map_arcgis"])) { $privileges["map_arcgis"] = stringToBool($gsValues['MAP_ARCGIS']); }
		if (!isset($privileges["map_yandex"])) { $privileges["map_yandex"] = stringToBool($gsValues['MAP_YANDEX']); }
		if (!isset($privileges["kml"])) { $privileges["kml"] = true; }
		if (!isset($privileges["dashboard"])) { $privileges["dashboard"] = true; }
		if (!isset($privileges["history"])) { $privileges["history"] = true; }
		if (!isset($privileges["reports"])) { $privileges["reports"] = true; }
		if (!isset($privileges["tachograph"])) { $privileges["tachograph"] = true; }
		...
		if (!isset($privileges["expenses"])) { $privileges["expenses"] = true; }
	...
	function changeObjectIMEI($old_imei, $new_imei)
	{
		...
		// gs_user_expenses
		$q = "UPDATE `gs_user_expenses` SET `imei`='".$new_imei."' WHERE `imei`='".$old_imei."'";
		$r = mysqli_query($ms, $q);

		// gs_user_share_position
		$q = "UPDATE `gs_user_share_position` SET `imei`='".$new_imei."' WHERE `imei`='".$old_imei."'";
		$r = mysqli_query($ms, $q);
		...
		// clear sensor cache
		$q = "UPDATE `gs_objects` SET `params`='' WHERE `imei`='".$new_imei."'";
		$r = mysqli_query($ms, $q);
	...
	function getObjectDriverFromSensor($user_id, $imei, $params)
	{
		...
		// blue id
		$sensor = getSensorFromType($imei, 'blueid');
		...
		if ($sensor != false)
		{
			...
			{
				if ((substr($key, 0, 3) == 'ble') && (substr($key, -3) == 'sig'))
				{
					$ble_array[$key] = $value;
				}
			}
			...
			foreach ($ble_array as $key => $value)
			{
				$driver_assign_id = substr($key, 3, 8);
	...
	function getObjectTrailerFromSensor($user_id, $imei, $params)
	{
		...
		if ($sensor != false)
		{
			...
			foreach ($ble_array as $key => $value)
			{
				$trailer_assign_id = substr($key, 3, 8);
	...
	function getObjectAccuracy($imei)
	{
		...
		else
		{
			...
			if (!isset($accuracy["route_length"])) { $accuracy["route_length"] = $default["route_length"]; }
			...
			if (!isset($accuracy["ign_fuel_cons_stops"])) { $accuracy["ign_fuel_cons_stops"] = $default["ign_fuel_cons_stops"]; }
    ...
    function sendObjectGPRSCommand($user_id, $imei, $name, $type, $cmd)
    {
        ...
        if ($num == 0)
        {
            ...
            $q2 = "SELECT gs_objects.*, gs_object_cmd_exec.*
                FROM gs_objects
            ...
            while($row = mysqli_fetch_array($r1))
            {
                ...
                {
                    $result1[] = array("cmd_id" => intval($row['cmd_id']),
                        ...
                        "cmd" => $row['cmd']);
                    $attr = array("data" =>  $row['cmd'],
                        "cmd_id" => intval($row['cmd_id'])
                    );
                    $q3 = "UPDATE `gs_object_cmd_exec` SET `status`='2' WHERE `cmd_id`='".$row["cmd_id"]."'";
                    $r2 = mysqli_query($ms, $q3);
                    $s = gps::commandSend($sessionId,  $row['deviceId'], "custom", json_encode($attr));
                }
            }
	...
	function mergeParamsBLE($old, $new, $is_ble, $remove_ble)
	{
		if (is_array($old) && is_array($new))
		{
			// remove old params BLE
			if (($is_ble) && (!$remove_ble))
			{
			...
			// check for params BLE and remove
			if ($remove_ble)
			{
	...
	function isMySQLOrderByValid($column, $order)
	{
		$column = caseToLower($column);
		$order = caseToLower($order);
	...
	function isMySQLLimitValid($start, $limit)
	{
