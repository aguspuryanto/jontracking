<?
    ...
    include('../server/traccar_api.php');
	checkUserSession();
	...
	if(@$_POST['cmd'] == 'add_object')
	{
		if (($_SESSION["manager_id"] == 0) && ($_SESSION["obj_add"] != 'false'))
		{
            ...
            $s = gps::deviceAdd($sessionId,  $imei, $imei);
	...
	if(@$_POST['cmd'] == 'edit_object')
	{
		if($_SESSION["obj_edit"] == 'true')
		{
			...
			$unassign_driver = $_POST["unassign_driver"];
			$accvirt = $_POST["accvirt"];
			$accvirt_cn = $_POST["accvirt_cn"];
			$forward_loc_data = $_POST["forward_loc_data"];
			$forward_loc_data_imei = $_POST["forward_loc_data_imei"];

			if (!checkUserToObjectPrivileges($user_id, $forward_loc_data_imei))
			{
			...
			$q = "UPDATE `gs_objects` SET 	`name`='".$name."',
											...
											`forward_loc_data`='".$forward_loc_data."',
			...
			//write log
			writeLog('object_op', 'Edit object: successful. q: '.$q);
	...
	if(@$_POST['cmd'] == 'load_object_data')
	{
		...
		while($row = mysqli_fetch_array($r))
		{
			...
			else
			{
				...
				if (!isset($accvirt_cn["param"])) { $accvirt_cn["param"] = $default["param"]; }
				if (!isset($accvirt_cn["accvirt_1_cn"])) { $accvirt_cn["accvirt_1_cn"] = $default["accvirt_1_cn"]; }
				if (!isset($accvirt_cn["accvirt_0_cn"])) { $accvirt_cn["accvirt_0_cn"] = $default["accvirt_0_cn"]; }
				if (!isset($accvirt_cn["accvirt_1_val"])) { $accvirt_cn["accvirt_1_val"] = $default["accvirt_1_val"]; }
				if (!isset($accvirt_cn["accvirt_0_val"])) { $accvirt_cn["accvirt_0_val"] = $default["accvirt_0_val"]; }
			}
			...
			$result[$imei] = array(	$row['protocol'],
									...
									$row['accvirt'],
									$accvirt_cn,
									$row['forward_loc_data'],
									$row['forward_loc_data_imei'],
									$sensors,
									$service,
									$custom_fields,
	...
	if(@$_GET['cmd'] == 'load_object_list')
	{
		...
		$q = "SELECT gs_objects.*, gs_user_objects.* , gs_user_object_groups.*
				FROM gs_objects
				INNER JOIN gs_user_objects ON gs_objects.imei = gs_user_objects.imei
				LEFT JOIN gs_user_object_groups ON gs_user_objects.group_id = gs_user_object_groups.group_id
				WHERE gs_user_objects.user_id='".$user_id."'";
		...
		if ($r)
		{
			...
			while($row = mysqli_fetch_array($r))
			{
				...
				if ($group_name != '')
				{
					$group_name = '<a href="#" onclick="settingsObjectGroupProperties(\''.$row['group_id'].'\');" title="Edit">'.$group_name.'</a>';
				}
				...
				$response->rows[$i]['cell']=array($row['name'],$imei,$group_name,$active,$object_expire_dt,$modify);
				$i++;
			}
		}
