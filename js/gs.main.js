...
function objectLoadList() {
	...
	for (var t in e.clearGridData(!0), settingsObjectData) {
		...
		if ("true" == a.active) {
			...
			r += '<div class="right"><div class="speed" id="object_speed_' + t + '">0 ' + la.UNIT_SPEED + "</div>",
				r += '<div class="engine" id="object_engine_' + t + '"></div>',
				r += '<div class="connection" id="object_connection_' + t + '">' + getConnectionIcon(0) + "</div></div></div>";
...
function settingsDeleteAccount(){
	var account_password = document.getElementById("settings_main_delete_password").value;
	var account_id = document.getElementById("settings_main_delete_account_id").value;
